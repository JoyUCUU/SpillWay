...
- (instancetype)initWithFrame:(CGRect)frame menuItem:(MenuItem *)menuItem {
    ...
    if (self) {
        ...
        imageSize = CGSizeMake(imageSize.width *0.7, imageSize.height *0.7);
//        if (kDevice_Is_iPhone6Plus) {
//            imageSize = CGSizeMake(imageSize.width*1.5 , imageSize.height*1.5);
//        }
//        if (kDevice_Is_iPhone6) {
//            imageSize = CGSizeMake(imageSize.width , imageSize.height);
//        }
//        if(kDevice_Is_iPhone4){
//            imageSize = CGSizeMake(imageSize.width *0.65, imageSize.height *0.65);
//        }
        
        UIView *imageBGView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, imageSize.width+20, imageSize.height+20)];
        imageBGView.backgroundColor=[UIColor whiteColor];
        if(menuItem.glowColor!=nil){
            imageBGView.backgroundColor=[UIColor clearColor];
        }
        imageBGView.layer.masksToBounds=YES;
        imageBGView.layer.cornerRadius=imageSize.width/2+10;
        
        self.iconImageView = [[GlowImageView alloc] initWithFrame:CGRectMake(10, 10, imageSize.width, imageSize.height)];
...
//        self.iconImageView.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.iconImageView.bounds));
        imageBGView.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(imageBGView.bounds));
        [imageBGView addSubview:self.iconImageView];
        [self addSubview:imageBGView];
        
        self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(imageBGView.frame), CGRectGetWidth(self.bounds), 35)];
